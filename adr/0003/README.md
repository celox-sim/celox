...
MIR (word-level SSA, virtual registers) + SpillDesc per vreg
  ↓ Live-range splitting at clobbers (UDiv/URem → RAX/RDX)
  ↓ Spilling (Braun & Hack extended MIN, constraint-aware k_eff)
  ↓ Live-range splitting at Fixed constraints (shift rhs → RCX)
...
#### Constraint-aware spilling

x86-64 の不規則なレジスタ制約（shift の RCX 固定、div/rem の RAX/RDX clobber）は、spilling と assignment の責務を明確に分離して対処する。

**設計原則**: スケーリング問題（live register 数百〜数千）と制約問題（RCX 固定等）は独立した問題であり、別フェーズで解く。

- **Spilling** の責務: 全体の圧力制御。制約による実効レジスタ数の減少を考慮する
- **Assignment** の責務: 物理レジスタの割り当て。制約を直接解決する

##### 実効レジスタ数 k_eff

各 program point で Fixed 制約が要求するレジスタ数だけ、実効的な割り当て可能レジスタ数を減らす:
...
UDiv/URem が clobber する RAX/RDX については、spilling 前に live-range splitting で対処する。clobber 命令をまたいで live な VReg を Mov で分割し、clobber 後の使用を fresh VReg に書き換える。

##### Live-range splitting at Fixed constraints

spilling 後・assignment 前に、Fixed 制約の use を1命令寿命に分離する post-pass を実行する。

**背景**: assignment の greedy coloring は VReg に対して global に PhysReg を割り当てる（`result.set(vreg, preg)` は VReg の全寿命に影響）。Fixed 制約の解決時に occupant を displacement すると、その VReg の過去の program point での割り当ても変わり、他の VReg と衝突する。

**解法**: Fixed 制約を持つ use の直前に `Mov fresh, original` を挿入し、fresh VReg だけが Fixed レジスタに割り当てられるようにする。元の VReg の割り当ては不変。

```
// Before splitting:
inst 0: v1 = ...         // v1 lives for many instructions
inst N: v3 = shl v2, v1  // v1 needs RCX (Fixed constraint)

// After splitting:
inst 0: v1 = ...         // v1 keeps its original register
inst N: v4 = mov v1      // fresh copy, 1-instruction lifetime
inst N+1: v3 = shl v2, v4  // v4 → RCX (safe, no global impact)
```

寿命が1命令以下（直前の命令で定義された場合）の VReg は既に安全なため split しない。

##### Assignment の RCX 回避

assignment の def 割り当て時、VReg の live range 内に shift 命令がある場合は RCX を blocked set に追加する。これにより shift 時点で RCX が空いていることを保証し、displacement を不要にする。
