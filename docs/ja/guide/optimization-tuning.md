...
::: tip TL;DR
デフォルト設定（production パス有効）が最も汎用的です。コンパイル時間やシミュレーション速度に具体的なボトルネックがある場合のみチューニングし、必ず実際のデザインでベンチマークしてください。
:::
...
| `branchify_mux` | 2-state mux の排他的なarm計算を、局所期待コストが有利な場合だけ分岐内へ移動（明示的opt-in） |

`branchify_mux` は `passOverrides: ["+sir:branchify_mux"]` で評価できます。
判断にはコンパイル時間やIRサイズではなく、同一条件のruntime A/Bを使ってください。
source DAGをSIRへloweringする段階でも収益性が証明できるmuxはCFG化され、
このSIRパスはlowering後に残った機会だけを補完します。
