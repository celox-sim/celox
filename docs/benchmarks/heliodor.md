...
By default this runs `test_soc_linux_boot` with synchronous Veryl AOT-C and then
Celox. Celox is timed out after `HELIODOR_CELOX_TIMEOUT_MULTIPLIER` times the
successful Veryl baseline for that test.
...
The default synchronous Veryl-CC runner separates generated-code throughput
from compiler latency:
...
Run the reproducible throughput comparison from a clean, committed Celox
checkout:
...
- Veryl simulator `0.20.2` pinned by the clean Celox `Cargo.lock` and a locked
  build rather than a CLI selected from `PATH` or `VERYL_BIN`;
- a clean, unchanged Celox `HEAD`, locked release/LTO builds of both timed
...
invocation. Both runners must exit successfully and each log exactly one exact
configuration record, one split timing record, and one full-pass result record.
The Celox configuration must be native/O2/`four_state=false`/
`compile_only=false`; the Veryl configuration must be synchronous AOT-C with
`aot_c_async=false`. Source manifests, checkout identities, and runner
...
a failure. GNU `timeout` with `--kill-after` and Python 3 are required.

The last pre-split measurements included an iterative non-LTO comparison of the
same `cy=9ae070` workload at `76.446 s` with Veryl-CC and `184.652 s` with
Celox. The old fixed gate was then run from clean Celox commit `e917489e` with its
...
therefore failed its then-current combined-process performance condition. Its
artifacts are in
`target/heliodor/results/gate_20260716T010312Z.tcVUZd`. Routine development
runs continue to use the non-LTO `heliodor-dev` profile. The split fixed gate is
run only after the implementation is committed and ready for the deliberate
final release/LTO qualification.
