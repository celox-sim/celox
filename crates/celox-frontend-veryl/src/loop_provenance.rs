...
pub struct LoopSourceId(pub usize);
...
pub struct LoopSource {
    ...
    pub identifier_token: TokenRange,
    pub identifier: StrId,
    pub statement_token: TokenRange,
    pub body_token: TokenRange,
    pub parent: Option<LoopSourceId>,
}

/// Source-level loops collected before analyzer pass 2 flattens constant loops.
#[derive(Debug, Clone, Default)]
pub struct LoopSourceTable {
    sources: Vec<LoopSource>,
    by_identifier_token: HashMap<TokenRange, Vec<LoopSourceId>>,
}

impl LoopSourceTable {
    pub fn collect<'a>(asts: impl IntoIterator<Item = &'a Veryl>) -> Self {
    ...
    pub fn source(&self, id: LoopSourceId) -> Option<&LoopSource> {
        self.sources.get(id.0)
    }

    /// Match the source table against analyzer IR without changing either side.
    pub fn match_unrolled(self, ir: &Ir) -> LoopProvenance {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct UnrolledLoopCandidate {
    pub source: LoopSourceId,
    pub component_index: usize,
    pub module_name: StrId,
    pub module_token: TokenRange,
    ...
    pub parent_hierarchy: Vec<StrId>,
    /// Execution order, not numeric order.
    pub iterations: Vec<UnrolledIteration>,
}
...
#[derive(Debug, Clone)]
pub struct LoopRecoveryCandidate {
    pub source: LoopSource,
    pub unrolled: UnrolledLoopCandidate,
}

impl LoopProvenance {
    /// Diagnostic/test API for the later semantic recovery phase.
    pub fn candidates(&self) -> &[UnrolledLoopCandidate] {
        &self.candidates
    }

    pub fn candidates_for_module(
        &self,
    ...
    pub fn is_consistent_with(&self, ir: &Ir) -> bool {
