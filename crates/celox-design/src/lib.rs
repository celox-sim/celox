...
pub struct RuntimeSchema<A> {
    ...
    /// State written by RTL execution units. External component outputs may
    /// not target these signals because that would create multiple drivers.
    pub rtl_write_roots: HashSet<A>,
}
...
impl<A> Default for RuntimeSchema<A> {
    fn default() -> Self {
        Self {
            ...
            rtl_write_roots: HashSet::default(),
        }
    }
}
