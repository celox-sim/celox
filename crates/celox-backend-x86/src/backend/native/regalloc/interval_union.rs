...
mod tests {
    ...
    #[test]
    fn dynamic_stack_union_merges_adjacent_ranges_independent_of_insertion_order() {
        let mut block = MBlock::new(BlockId(0));
        block.push(MInst::Return);
        let mut function = function(0, vec![block]);
        let cfg = normalize(&mut function);
        let slot = |phase| SlotIndex::stable(0, 0, phase).unwrap();
        let left = LiveSegment {
            block: BlockId(0),
            start: slot(0),
            end: slot(1),
        };
        let middle = LiveSegment {
            block: BlockId(0),
            start: slot(1),
            end: slot(2),
        };
        let right = LiveSegment {
            block: BlockId(0),
            start: slot(2),
            end: slot(3),
        };

        let mut first = DynamicIntervalMatrix::new(&cfg).unwrap();
        for (bundle, segment) in [
            (AllocationBundleId(0), left),
            (AllocationBundleId(1), right),
            (AllocationBundleId(2), middle),
        ] {
            let range = first.make_range(vec![segment]).unwrap();
            assert_eq!(
                first.first_available_validated(range.validated()).unwrap(),
                0
            );
            first
                .assign_validated(bundle, 0, range.validated())
                .unwrap();
        }
        first.verify().unwrap();

        let mut second = DynamicIntervalMatrix::new(&cfg).unwrap();
        for (bundle, segment) in [
            (AllocationBundleId(1), right),
            (AllocationBundleId(2), middle),
            (AllocationBundleId(0), left),
        ] {
            let range = second.make_range(vec![segment]).unwrap();
            second
                .assign_validated(bundle, 0, range.validated())
                .unwrap();
        }
        ...
        assert_eq!(
            first
                .first_available_validated(covering.validated())
                .unwrap(),
            1
        );
        let before = first.clone();
        let error = first
            .assign_validated(AllocationBundleId(3), 0, covering.validated())
            .unwrap_err();
        assert_eq!(error.rule, "INTERVAL_UNION.INTERFERENCE");
