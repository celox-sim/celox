        ...
        if self.constant(initial_index) != Some(0)
            ...
            || self.func.blocks.iter().any(|block| {
                !blocks.contains(&block.id)
                    && block.successors().iter().any(|target| {
                        blocks.contains(target) && (block.id != entry || *target != header.id)
                    })
            })
        {
            return None;
        ...
        for block in &self.func.blocks {
            if blocks.contains(&block.id) {
                if block.insts.iter().any(|inst| {
                    !matches!(inst, MInst::Branch { .. } | MInst::Jump { .. }) && !pure(inst)
                }) {
                    return None;
                }
            } else if block
