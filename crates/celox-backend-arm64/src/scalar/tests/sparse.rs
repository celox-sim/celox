...
#[test]
fn full_dirty_words_preserve_chunk_order_for_overlapping_storage() {
    for bytes in [17, 32, 512] {
        for destination in [0, 7, 11, 19, 600] {
            for four_state in [false, true] {
                let row = [
                    3,
                    destination,
                ...
                let jit = emit_with_pages(StatePageBases::default(), |ops| {
                    emit_sparse_commit(
                        ops,
                        3,
                        destination as i32,
                ...
                let mut state = (0..2064)
                    .map(|index| (index as u8).wrapping_mul(73).wrapping_add(17))
                    .collect::<Vec<_>>();
                state[2048..2064].fill(0xff);
                let mut expected = state.clone();
                reference_commit(&mut expected, &row);
                assert_eq!(unsafe { (jit.fn_ptr)(state.as_mut_ptr()) }, 0);
                assert_eq!(
                    state, expected,
                    "bytes={bytes} dst={destination} four_state={four_state}"
                );
            }
        }
    }
