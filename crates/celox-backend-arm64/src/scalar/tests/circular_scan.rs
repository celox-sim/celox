...
use crate::mir_opt::circular_scan::run;
...
pub(crate) fn fixture(width: u32, blocker: u8) -> MFunction {
    ...
    let mut function = MFunction::for_isel(vregs, vec![SpillDesc::transient(); 30]);
...
#[test]
fn circular_bitmap_scans_keep_all_external_entries() {
    let mut function = fixture(32, 0);
    let mut root = MBlock::new(BlockId(8));
    root.insts = std::mem::take(&mut function.blocks[0].insts);
    root.insts.pop();
    root.push(MInst::Branch {
        cond: VReg(5),
        true_bb: BlockId(0),
        false_bb: BlockId(7),
    });
    function.blocks[0].push(MInst::Jump { target: BlockId(1) });
    let mut alternative = MBlock::new(BlockId(7));
    alternative.push(MInst::Jump { target: BlockId(1) });
    for phi in &mut function.blocks[1].phis {
        phi.sources.push((BlockId(7), VReg(0)));
    }
    function.blocks.insert(0, root);
    function.blocks.push(alternative);
    celox_backend_common::regalloc::analyze_live_intervals(
        &crate::regalloc::build_facts(&function).unwrap(),
    )
    .unwrap();
