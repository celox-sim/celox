...
impl SIRTranslator {
    ...
    pub(super) fn translate_binary_inst(
        ...
        if common_logical_width <= 64 {
            ...
            let l_is_signed = if any_divrem {
                ...
                signed_comparison
            } else if matches!(op, BinaryOp::Shr) {
                // A logical right shift must zero-fill from the operand's
                // logical width.  The SIR register can still carry the
                // source declaration's signedness after an equal-width
                // unsigned context conversion; sign-extending it to the
                // Cranelift physical type would turn `0xf9 >> 1` into
                // `0xfc` after truncation instead of `0x7c`.
                false
            } else {
                state.register_map[lhs].is_signed() || matches!(op, BinaryOp::Sar)
            };
            let r_is_signed = signed_divrem || signed_comparison;
            ...
            if self.options.four_state {
                ...
                let res_m = match op {
                    ...
                    BinaryOp::EqWildcard | BinaryOp::NeWildcard => {
                        ...
                        let definite_compare = state.builder.ins().band(compare_mask, l_definite);

                        // Check for definite mismatch at those positions
                        let l_xor_r = state.builder.ins().bxor(l, r);
                        let mismatch_bits = state.builder.ins().band(l_xor_r, definite_compare);
                        let has_definite_mismatch =
                            state
                                .builder
                                .ins()
                                .icmp(IntCC::NotEqual, mismatch_bits, zero);

                        // LHS X at non-wildcard positions
                        let x_at_compared = state.builder.ins().band(l_m, compare_mask);
                        let has_x_at_compared =
                            state
                                .builder
                                .ins()
                                .icmp(IntCC::NotEqual, x_at_compared, zero);
                    ...
                    BinaryOp::LogicAnd => {
                        ...
                        let l_def_false =
                            state.builder.ins().icmp(IntCC::Equal, l_val_or_mask, zero);
                        let r_def_false =
                            state.builder.ins().icmp(IntCC::Equal, r_val_or_mask, zero);
                        let either_def_false = state.builder.ins().bor(l_def_false, r_def_false);
                        ...
                        let all_ones = state.builder.ins().iconst(common_ty, -1);
                        let conservative = state.builder.ins().select(any_x, all_ones, zero);
                        state
                            .builder
                            .ins()
                    ...
                    BinaryOp::LogicOr => {
                        ...
                        let l_def_true =
                            state
                                .builder
                                .ins()
                                .icmp(IntCC::NotEqual, l_definite_v, zero);
                        let r_def_true =
                            state
                                .builder
                                .ins()
                                .icmp(IntCC::NotEqual, r_definite_v, zero);
                        let either_def_true = state.builder.ins().bor(l_def_true, r_def_true);
            ...
            if is_shift && num_chunks >= MEM_SHIFT_THRESHOLD {
                ...
                for (i, &v) in src_chunks.iter().enumerate() {
                    state
                        .builder
                        .ins()
                        .store(MemFlags::new(), v, l_addr, (i * 8) as i32);
                }
                ...
                for i in src_chunks.len()..num_chunks {
                    state
                        .builder
                        .ins()
                        .store(MemFlags::new(), zero_pad, l_addr, (i * 8) as i32);
                }
                ...
                let mask_addr = if self.options.four_state {
                    ...
                    for (i, &m) in l_masks.iter().enumerate() {
                        state
                            .builder
                            .ins()
                            .store(MemFlags::new(), m, l_mask_addr, (i * 8) as i32);
                    }
                ...
                if final_num_chunks >= MEM_SHIFT_THRESHOLD {
                    ...
                    let res_chunks: Vec<Value> = (0..final_num_chunks)
                        .map(|i| {
                ...
                if self.options.four_state {
                    ...
                    let mut res_masks = match op {
                        ...
                        BinaryOp::EqWildcard | BinaryOp::NeWildcard => {
                            ...
                            let all_ones = state.builder.ins().iconst(types::I64, -1i64);
                            let x_mask = state.builder.ins().select(has_x, all_ones, zero);
                            let mask_val = state.builder.ins().select(has_mismatch, zero, x_mask);
                            ...
                            while new_res.len() < final_num_chunks {
                                new_res.push(state.builder.ins().iconst(types::I64, 0));
                            }
                            res_chunks = new_res;
                        ...
                        BinaryOp::LogicAnd | BinaryOp::LogicOr => {
                            ...
                            let mut l_val_or = state.builder.ins().iconst(types::I64, 0);
                            let mut r_val_or = state.builder.ins().iconst(types::I64, 0);
                            let mut l_mask_or = state.builder.ins().iconst(types::I64, 0);
                            let mut r_mask_or = state.builder.ins().iconst(types::I64, 0);
                            for i in 0..num_chunks {
                                let lv = get_chunk_as_i64(state.builder, &l_chunks, i);
                                let rv = get_chunk_as_i64(state.builder, &r_chunks, i);
                                let lm = get_chunk_as_i64(state.builder, &l_masks, i);
                                let rm = get_chunk_as_i64(state.builder, &r_masks, i);
                                l_val_or = state.builder.ins().bor(l_val_or, lv);
                                r_val_or = state.builder.ins().bor(r_val_or, rv);
                                l_mask_or = state.builder.ins().bor(l_mask_or, lm);
                                r_mask_or = state.builder.ins().bor(r_mask_or, rm);
                            }

                            let zero = state.builder.ins().iconst(types::I64, 0);
                            let any_x_all = state.builder.ins().bor(l_mask_or, r_mask_or);
                            let has_x = state.builder.ins().icmp(IntCC::NotEqual, any_x_all, zero);
                            let all_ones = state.builder.ins().iconst(types::I64, -1i64);
                            let conservative = state.builder.ins().select(has_x, all_ones, zero);

                            let dominant = if matches!(op, BinaryOp::LogicAnd) {
                                // 0 is dominant: check if either operand is definite false
                                // definite false = (v | m) == 0
                                let l_vm = state.builder.ins().bor(l_val_or, l_mask_or);
                                let r_vm = state.builder.ins().bor(r_val_or, r_mask_or);
                                let l_def_false =
                                    state.builder.ins().icmp(IntCC::Equal, l_vm, zero);
                                let r_def_false =
                                    state.builder.ins().icmp(IntCC::Equal, r_vm, zero);
                                ...
                                let mut l_definite_or = state.builder.ins().iconst(types::I64, 0);
                                let mut r_definite_or = state.builder.ins().iconst(types::I64, 0);
                                for i in 0..num_chunks {
                                    let lv = get_chunk_as_i64(state.builder, &l_chunks, i);
                                    let lm = get_chunk_as_i64(state.builder, &l_masks, i);
                                    let rv = get_chunk_as_i64(state.builder, &r_chunks, i);
                                    ...
                                    l_definite_or = state.builder.ins().bor(l_definite_or, l_def);
                                    r_definite_or = state.builder.ins().bor(r_definite_or, r_def);
                                }
                                let l_def_true =
                                    state
                                        .builder
                            ...
                            let mask_val = state.builder.ins().select(dominant, zero, conservative);
                            vec![mask_val; final_num_chunks]
                        }
                    ...
                    if last_chunk_bits != 0 && !res_masks.is_empty() {
                        ...
                        let width_mask = state.builder.ins().iconst(types::I64, width_mask_val);
    ...
    pub(super) fn translate_unary_inst(
        ...
        if common_logical_width <= 64 {
            ...
            let res_v = match op {
                ...
                UnaryOp::Ident | UnaryOp::ToTwoState => r,
                UnaryOp::BitNot => {
            ...
            if self.options.four_state {
                ...
                let res_m = match op {
                    ...
                    UnaryOp::ToTwoState => state.builder.ins().iconst(common_ty, 0),
                    ...
                    UnaryOp::And => {
                        ...
                        let zero = state.builder.ins().iconst(common_ty, 0);
                        let has_definite_zero =
                            state
                                .builder
                                .ins()
                                .icmp(IntCC::NotEqual, definite_zeros_masked, zero);
                        let has_any_x = state.builder.ins().icmp(IntCC::NotEqual, r_m, zero);
                        let all_ones = state.builder.ins().iconst(common_ty, -1);
                        let x_mask = state.builder.ins().select(has_any_x, all_ones, zero);
                ...
                let normalized_v = match op {
                    UnaryOp::Ident => final_res_v,
                    UnaryOp::ToTwoState => {
                        let final_src_m = if common_ty.bits() > dst_ty.bits() {
                            state.builder.ins().ireduce(dst_ty, r_m)
                        } else {
                            r_m
                        };
                        let defined = state.builder.ins().bnot(final_src_m);
                        state.builder.ins().band(final_res_v, defined)
                    }
                    _ => state.builder.ins().bor(final_res_v, final_res_m),
                };
                state.regs.insert(
                    *dst,
                    TransValue::FourState {
            ...
            if self.options.four_state {
                ...
                let mut res_masks = match op {
                    ...
                    UnaryOp::LogicNot | UnaryOp::Or => {
                        ...
                        for i in 0..num_chunks {
                            ...
                            accumulated_definite_ones = state
                                .builder
                                .ins()
                    ...
                    UnaryOp::And => {
                        ...
                        for i in 0..num_chunks {
                            ...
                            if i == num_chunks - 1 {
                                ...
                                if remaining < 64 {
                                    let last_chunk_mask = ((1u64 << remaining) - 1) as i64;
                                    let mask_val =
                            ...
                            accumulated_definite_zeros = state
                                .builder
                                .ins()
                        ...
                        let all_ones = state.builder.ins().iconst(types::I64, -1i64);
                        let x_mask = state.builder.ins().select(has_any_x, all_ones, zero);
                        let mask_val = state.builder.ins().select(has_definite_zero, zero, x_mask);
                        vec![mask_val; final_num_chunks]
                    }
                ...
                let normalized: Vec<_> = match op {
                    UnaryOp::Ident => res_chunks,
                    UnaryOp::ToTwoState => res_chunks
                        .iter()
                        .enumerate()
                        .map(|(index, &value)| {
                            let source_mask = get_chunk_as_i64(state.builder, &r_masks, index);
                            let defined = state.builder.ins().bnot(source_mask);
                            state.builder.ins().band(value, defined)
                        })
                        .collect(),
                    _ => res_chunks
                        .iter()
    ...
    ///
    /// A condition is definitely true when any known bit is one, unknown when
    /// it has no known-one bit but contains X/Z, and otherwise false.  For an
    /// unknown condition, identical four-state arm bits are preserved and only
    /// differing bits become X, as required by SystemVerilog's conditional
    /// operator semantics.
    pub(super) fn translate_mux_inst(
        ...
        let zero = state.builder.ins().iconst(types::I64, 0);
        let cond_chunks = state.regs[cond].load_value_chunks(state.builder);
        let cond_masks = state.regs[cond]
            .load_mask_chunks(state.builder)
            .unwrap_or_default();
        let cond_n_chunks = cond_width.div_ceil(64).max(1);
        let mut known_one_bits = zero;
        let mut unknown_bits = zero;
        for index in 0..cond_n_chunks {
            let mut value = get_chunk_as_i64(state.builder, &cond_chunks, index);
            ...
            known_one_bits = state.builder.ins().bor(known_one_bits, known_ones);
            unknown_bits = state.builder.ins().bor(unknown_bits, mask);
        }
        let cond_is_true = state
            .builder
            .ins()
            .icmp(IntCC::NotEqual, known_one_bits, zero);
        let cond_has_unknown = state
            .builder
            .ins()
            .icmp(IntCC::NotEqual, unknown_bits, zero);
        let cond_is_not_true = state.builder.ins().icmp(IntCC::Equal, known_one_bits, zero);
        let cond_is_unknown = state.builder.ins().band(cond_has_unknown, cond_is_not_true);
        ...
        if d_width <= 64 {
            ...
            let selected_value = state.builder.ins().select(cond_is_true, tv, ev);
            ...
            if self.options.four_state {
                ...
                let selected_mask = state.builder.ins().select(cond_is_true, tm, em);

                let value_diff = state.builder.ins().bxor(tv, ev);
                let mask_diff = state.builder.ins().bxor(tm, em);
                let diff = state.builder.ins().bor(value_diff, mask_diff);
                let unknown_value = state.builder.ins().bor(tv, diff);
                let unknown_mask = state.builder.ins().bor(tm, diff);
                let result =
                    state
                        .builder
                        .ins()
                        .select(cond_is_unknown, unknown_value, selected_value);
                let result_mask =
                    state
                        .builder
                        .ins()
                        .select(cond_is_unknown, unknown_mask, selected_mask);
                let result = apply_d_width_mask(state, result, ty, d_width);
                let result_mask = apply_d_width_mask(state, result_mask, ty, d_width);

                state.regs.insert(
                    *dst,
                    TransValue::FourState {
                        values: vec![result],
                        masks: vec![result_mask],
                    },
                );
            } else {
                let result = apply_d_width_mask(state, selected_value, ty, d_width);
            ...
            for i in 0..n_chunks {
                let tv = get_chunk_as_i64(state.builder, &tv_chunks, i);
                let ev = get_chunk_as_i64(state.builder, &ev_chunks, i);
                let selected_value = state.builder.ins().select(cond_is_true, tv, ev);
                let mut result = selected_value;

                if self.options.four_state {
                    let tm = get_chunk_as_i64(state.builder, &tm_chunks, i);
                    let em = get_chunk_as_i64(state.builder, &em_chunks, i);
                    let selected_mask = state.builder.ins().select(cond_is_true, tm, em);
                    let value_diff = state.builder.ins().bxor(tv, ev);
                    let mask_diff = state.builder.ins().bxor(tm, em);
                    let diff = state.builder.ins().bor(value_diff, mask_diff);
