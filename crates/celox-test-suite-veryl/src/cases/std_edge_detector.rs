...
cases! { StandardLibrary, "std_edge_detector";
    ...
    fn test_edge_detector_basic(sim) {
        @setup { let code = format!("{}\n{TOP_W1}", super::std_source(&["edge_detector", "edge_detector.veryl"])); }
...
"#;
let code = format!("{}\n{top}", super::std_source(&["edge_detector", "edge_detector.veryl"])); }
        @build Design::new(&code, "Top");
    let clk = sim.event("clk");
    let rst = sim.signal("rst");
    let i_data = sim.signal("i_data");
