...
fn test_comb_statement_output_copyout_obeys_named_argument_order(sim) {
    // The legacy case ID is retained. Neither positional nor named binding
    // establishes the order of blocking copy-outs to an aliased destination.

    @build Design::new(r#"
        module Top (
            d: input logic<8>, positional: output logic<8>, named: output logic<8>,
            distinct_first: output logic<8>, distinct_second: output logic<8>,
        ) {
            ...
            always_comb {
                ...
                split(second: distinct_second, x: d, first: distinct_first);
            }
        }
    "#, "Top");
    let d = sim.signal("d");
    ...
    let distinct_first = sim.signal("distinct_first");
    let distinct_second = sim.signal("distinct_second");

    for value in [0u8, 7, 254, 255] {
        sim.modify(|io| io.set(d, value)).unwrap();
        let allowed = [value.wrapping_add(1), value.wrapping_add(2)];
        for signal in [positional, named] {
            let actual = sim.get_as::<u8>(signal);
            assert!(allowed.contains(&actual), "copy-out must leave one complete output value: expected {allowed:?}, got {actual}");
        }
        assert_eq!(sim.get_as::<u8>(distinct_first), value.wrapping_add(1));
        assert_eq!(sim.get_as::<u8>(distinct_second), value.wrapping_add(2));
    }
}
