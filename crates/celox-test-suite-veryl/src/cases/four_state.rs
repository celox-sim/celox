...
fn test_four_state_wide_shifts(sim) {
    ...
    // IEEE 1800-2023 11.4.10: shifts preserve retained X and Z bits.
    // Case 1: Shift by 0; bits 71:64 contain alternating X/Z.
    ...
    assert_eq!(v_shr, all_x, "Shift by X should result in all-X payload");
...
// ==========================================================================
// Exercise scalar, multiword, and memory-backed shifts, including narrowing.
fn test_four_state_shifts_preserve_xz(sim) {
    @setup {
        let widths = [8usize, 64, 65, 128, 256];
        let mut ports = vec!["sh: input logic<16>".to_owned()];
        let mut assigns = String::new();
        for width in widths {
            ports.push(format!("a_{width}: input logic<{width}>"));
            for output in ["shr", "shl", "constant"] {
                ports.push(format!("{output}_{width}: output logic<{width}>"));
            }
            ports.push(format!("narrow_{width}: output logic<4>"));
            assigns.push_str(&format!("assign shr_{width} = a_{width} >> sh;\nassign shl_{width} = a_{width} << sh;\nassign constant_{width} = a_{width} >> 1;\nassign narrow_{width} = a_{width} >> sh;\n"));
        }
        let code = format!("module Top ({}) {{{assigns}}}", ports.join(",\n"));
    }
    @build Design::new(&code, "Top").four_state(true);
    let sh = sim.signal("sh");
    for width in widths {
        let a = sim.signal(&format!("a_{width}"));
        let shr = sim.signal(&format!("shr_{width}"));
        let shl = sim.signal(&format!("shl_{width}"));
        let constant = sim.signal(&format!("constant_{width}"));
        let narrow = sim.signal(&format!("narrow_{width}"));
        let limit = (BigUint::from(1u8) << width) - BigUint::from(1u8);
        let payload = (BigUint::from(0xaau8) << (width - 8))
            | BigUint::from(if width == 8 { 0 } else { 0x55u8 });
        let mask = BigUint::from(0xffu8) << (width - 8);
        for amount in [0usize, 1, width / 2, width - 1] {
            sim.modify(|io| {
                io.set_four_state(a, payload.clone(), mask.clone());
                io.set(sh, amount as u16);
            }).unwrap();
            assert_eq!(sim.get_four_state(shr), (&payload >> amount, &mask >> amount), "shr width={width} amount={amount}");
            assert_eq!(sim.get_four_state(shl), ((&payload << amount) & &limit, (&mask << amount) & &limit), "shl width={width} amount={amount}");
            assert_eq!(sim.get_four_state(constant), (&payload >> 1, &mask >> 1), "constant width={width}");
            assert_eq!(sim.get_four_state(narrow), ((&payload >> amount) & BigUint::from(15u8), (&mask >> amount) & BigUint::from(15u8)), "narrow width={width} amount={amount}");
        }
        // Both 1z and 1x must produce all X, despite the definite one bit.
        for count in [2u8, 3] {
            sim.modify(|io| io.set_four_state(sh, count.into(), 1u8.into())).unwrap();
            for output in [shr, shl] {
                assert_eq!(sim.get_four_state(output), (limit.clone(), limit.clone()), "unknown count width={width}");
            }
            assert_eq!(sim.get_four_state(narrow), (15u8.into(), 15u8.into()));
        }
    }
}
