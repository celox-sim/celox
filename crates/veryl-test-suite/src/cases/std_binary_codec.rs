...
cases! { StandardLibrary, "std_binary_codec";
...
"#;
let code = format!("{}\n{top}", super::std_source(&["binary_enc_dec", "binary_encoder.veryl"])); }
...
"#;
let code = format!("{}\n{top}", super::std_source(&["binary_enc_dec", "binary_decoder.veryl"])); }
        @build Design::new(&code, "Top");
...
let code = format!(
    "{}\n{}\n{top}",
    super::std_source(&["binary_enc_dec", "binary_encoder.veryl"]),
    super::std_source(&["binary_enc_dec", "binary_decoder.veryl"]),
); }
        @build Design::new(&code, "Top");
    let i_unary = sim.signal("i_unary");
    let o_unary = sim.signal("o_unary");
