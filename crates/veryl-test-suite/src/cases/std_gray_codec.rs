...
cases! { StandardLibrary, "std_gray_codec";
...
let code = format!(
    "{}\n{}\n{top}",
    super::std_source(&["gray", "gray_encoder.veryl"]),
    super::std_source(&["gray", "gray_decoder.veryl"]),
); }
...
"#;
let code = format!("{}\n{top}", super::std_source(&["gray", "gray_encoder.veryl"])); }
        @build Design::new(&code, "Top");
    let i_bin = sim.signal("i_bin");
    let o_gray = sim.signal("o_gray");
