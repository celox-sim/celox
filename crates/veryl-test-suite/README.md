...
Neither adapter treats an exit code or generic error prefix alone as compilation
rejection. Both require recognized source diagnostics naming an emitted file
and line, and check the remaining log for unrecognized or mixed failures. Empty
logs, error summaries alone, internal errors, I/O failures, unsupported tool
features, and timeout/tool-launch statuses remain build failures. Recognition
covers the retained negative-fixture diagnostics; new diagnostic formats must
be reviewed before they can satisfy an expected rejection. The full build log
remains available for investigation. Verilator's retained rejection is an
assignment type error; the seven accepted dynamic output connections still
